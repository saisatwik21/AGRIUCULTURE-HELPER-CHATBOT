...
def record(request):
    if request.method == "POST":
        ...
        path = os.path.abspath(os.getcwd())+'/ChatBotApp/static/'
        print("path: " + path)
        input_file = os.path.join(path, "record.wav")
        output_file = os.path.join(path, "record1.wav")

        ffmpeg_path = os.path.join(path, "ffmpeg.exe")
        result = subprocess.run([ffmpeg_path, "-i", input_file, output_file], capture_output=True)
